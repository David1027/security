...
public class ImageCodeUtils {
   ...
   * 创建图形验证码对象
   *
   * @return ImageCode图形验证码对象
   */
  public static ImageCode createImageCode(int width, int height, String verifyCode, int expireIn) {
    // 绘制一张内存中图片
    ...
    // 设置输出字体
    graphics2d.setFont(new Font("黑体", Font.BOLD, 18));
    ...
    for (int i = 0; i < verifyCode.length(); i++) {
      // 随机颜色
      graphics2d.setColor(
          new Color(20 + random.nextInt(80), 20 + random.nextInt(80), 20 + random.nextInt(80)));
      // 旋转 -30 --- 30度
  ...
  /**
   * 根据给定的一批字符从中随机获取指定个数的字符
   *
   * @param base 给定的字符挑选项
   * @param count 指定获取的字符个数
   * @return
   */
   ...
   * 随机生成指定个数的字母或数字
   *
   * @param count 指定生成字母或数的个数
   */
  public static String getRandomWord(int count) {
    String base = "0123456789ABCDEFGHJKMNPQRSTUVWXYZ";
